...
class FewShotPredictor(RiskPredictor):
    ...
    def prepare_data(
        ...
        if self.nb_shots > 0:
            ...
            df_controls = df_train[~df_train[phenotype]].sample(
                n=nb_controls, random_state=split_seed
            )
            df_cases = df_train[df_train[phenotype]].sample(
                n=nb_cases, random_state=split_seed
            )
            few_shots = (
                pd.concat([df_controls, df_cases]).sample(frac=1).reset_index(drop=True)
            )

            few_shots["prompt"] = few_shots.apply(
                lambda x: "".join(
                    [
                        self.system_prompt,
                        serialize_features(x, features),
                        self.instruction(phenotype),
                    ]
                ),
                axis=1,
            )
            few_shots["prompt"] = few_shots.apply(
                lambda x: "".join([x["prompt"], "Yes."])
                if x[phenotype]
                else "".join([x["prompt"], "No."]),
                axis=1,
            )
    ...
    def compute_scores(
        ...
        with torch.no_grad():
            self.y_scores = []
            for i in tqdm(
                range(0, len(prompts), self.batch_size), desc="Processing batches"
            ):
                # Create batch:
                ...
                # Compute risk scores:
                positive_probs = self._get_next_token_probs(
                    output, self.positive_tokens, debug=self.debug if i == 0 else False
                )
                negative_probs = self._get_next_token_probs(
                    output, self.negative_tokens
                )
    ...
    def _get_next_token_probs(
        self, output, target_tokens: List[str], debug=False
    ) -> float:
        # Encode target words:
        ...
        if debug:
            ...
            token_labels = [
                self.tokenizer.decode([token_id]) for token_id in token_ids[:20]
            ]
            for score, label, token_id in zip(scores, token_labels, token_ids):
                logger.debug(
                    repr("{:.2f} {} {}".format(score.item(), label, token_id.item()))
                )
