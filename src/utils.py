...
def load_llm(model_id, freeze=True, inference=True, torch_dtype=torch.float16):
    tokenizer = AutoTokenizer.from_pretrained(model_id, device_map="auto", padding_side='left')
    ...
    if torch.cuda.is_available():
        for i in range(torch.cuda.device_count()):
            logger.info("Memory allocated on cuda device {}: {:.1f} GB".format(i, torch.cuda.memory_allocated(i)*1e-9))
...
def is_valid_date(value):
    return not (pd.isna(value) or value in ["1900-01-01", "1901-01-01", "1902-02-02", "1903-03-03", "1909-09-09", "2037-07-07"])