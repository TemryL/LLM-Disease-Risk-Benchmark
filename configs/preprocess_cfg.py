...
features = [
    ...
    Feature(name="Sex", field_id="31", unit=None, is_valid=lambda x: x in [0, 1], decode_map={0:"Male", 1:"Female"}),
    Feature(name="BMI", field_id="21001", unit="kg/m²", is_valid=lambda x: x >= 0),
    Feature(name="HDL cholesterol", field_id="30760", unit="mmol/L", is_valid=lambda x: x >= 0),
    Feature(name="LDL cholesterol", field_id="30780", unit="mmol/L", is_valid=lambda x: x >= 0),
    Feature(name="Total cholesterol", field_id="30690", unit="mmol/L", is_valid=lambda x: x >= 0),
    Feature(name="Triglycerides", field_id="30870", unit="mmol/L", is_valid=lambda x: x >= 0),
    Feature(name="Diastolic blood pressure", field_id="4079", unit="mmHg", is_valid=lambda x: x >= 0),
    Feature(name="Ever smoked", field_id="20160", unit=None, is_valid=lambda x: x in [0, 1], decode_map={0:"No", 1:"Yes"}),
    Feature(name="Snoring", field_id="1210", unit=None, is_valid=lambda x: x in [1, 2], decode_map={1:"Yes", 2:"No"}),
    Feature(name="Insomnia", field_id="1200", unit=None, is_valid=lambda x: x in [1, 2, 3], decode_map={1:"Never/rarely", 2:"Sometimes", 3:"Usually"}),
    Feature(name="Daytime napping", field_id="1190", unit=None, is_valid=lambda x: x in [1, 2, 3], decode_map={1:"Never/rarely", 2:"Sometimes", 3:"Usually"}),
    Feature(name="Sleep duration", field_id="1160", unit="hours/day", is_valid=lambda x: x >= 0),
    Feature(name="Chronotype", field_id="1180", unit=None, is_valid=lambda x: x in [1, 2, 3, 4], decode_map={1:"Definitely a 'morning' person", 2:"More a 'morning' than 'evening' person", 3:"More an 'evening' than a 'morning' person", 4:"Definitely an 'evening' person"}),
]
...
phenotype_matching_rules = {
    ...
    "Diabetes": [
        ...
        ("41270", lambda x: check_icd_code(x, ["E10", "E11", "E12", "E13", "E14"])),    # ICD-10
    ...
    "Hay-fever & Eczema": [
        ("41270", lambda x: check_icd_code(x, [f"L2{i}" for i in range(10)] + ["L30"])),  # ICD-10
    ...
    "Osteoarthritis": [
        ("41270", lambda x: check_icd_code(x, ["M15", "M16", "M17", "M18", "M19"])),  # ICD-10
    ...
    "Pneumonia": [
        ("41271", lambda x: check_icd_code(x, ["401", "480", "481", "482", "483", "484", "486"])),  # ICD-9
        ("41270", lambda x: check_icd_code(x, ["J12", "J13", "J14", "J15", "J16", "J17", "J18"])),  # ICD-10
    ],
    "Stroke": [
